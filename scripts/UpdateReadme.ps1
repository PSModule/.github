$owner = 'PSModule'
$rawRepos = gh repo list $owner --json name, description --limit 100 | ConvertFrom-Json
$repos = $rawRepos | ForEach-Object {
    $rawRepo = $_
    $properties = gh api /repos/$owner/$($rawRepo.name)/properties/values | ConvertFrom-Json
    $properties | Where-Object { $_.property_name -eq 'Type' } | ForEach-Object {
        $type = $_.value
        [pscustomobject]@{
            Name        = $rawRepo.name
            Owner       = $owner
            Description = $rawRepo.description
            Type        = $type
        }
    }
}
$repos | Sort-Object Type, Name

$moduleTableRowTemplate = @'
    <tr>
        <td><a href="https://github.com/{{ OWNER }}/{{ NAME }}">{{ NAME }}</a></td>
        <td>{{ DESCRIPTION }}.
            <br>
            <a href="https://github.com/{{ OWNER }}/{{ NAME }}/issues"><img src="https://img.shields.io/github/issues-raw/{{ OWNER }}/{{ NAME }}?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Issues"></a>
            <a href="https://github.com/{{ OWNER }}/{{ NAME }}/pulls"><img src="https://img.shields.io/github/issues-pr-raw/{{ OWNER }}/{{ NAME }}?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Pull Requests"></a>
            <a href="https://github.com/{{ OWNER }}/{{ NAME }}/stargazers"><img src="https://img.shields.io/github/stars/{{ OWNER }}/{{ NAME }}?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Stars"></a>
            <a href="https://github.com/{{ OWNER }}/{{ NAME }}/watchers"><img src="https://img.shields.io/github/watchers/{{ OWNER }}/{{ NAME }}?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Watchers"></a>
            <a href="https://github.com/{{ OWNER }}/{{ NAME }}/forks"><img src="https://img.shields.io/github/forks/{{ OWNER }}/{{ NAME }}?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Forks"></a>
            <a href="https://www.powershellgallery.com/packages/{{ NAME }}/"><img src="https://img.shields.io/powershellgallery/dt/{{ NAME }}?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="PowerShell Gallery Downloads"></a>
        </td>
        <td>
            <a href="https://github.com/{{ OWNER }}/{{ NAME }}/releases/latest"><img src="https://img.shields.io/github/v/release/{{ OWNER }}/{{ NAME }}?style=flat-square&logo=github&logoColor=a0a0a0&label=&labelColor=505050&color=blue" alt="GitHub release (with filter)"></a>
            <a href="https://www.powershellgallery.com/packages/{{ NAME }}/"><img src="https://img.shields.io/powershellgallery/v/{{ NAME }}?style=flat-square&logo=data:image/svg%2bxml;base64,&label=&labelColor=505050&color=blue" alt="PowerShell Gallery Version"></a>
        </td>
    </tr>
'@
$moduleTableTemplate = @'
<!-- MODULE_LIST_START -->

<table>
    <tr>
        <th width="10%">Name</th>
        <th width="80%">Description</th>
        <th width="10%">Version</th>
    </tr>
    {0}
</table>

<!-- MODULE_LIST_END -->
'@
$moduleTable = $repos | New-MDTable

$actionTableRowTemplate = @'
    <tr>
        <td><a href="https://github.com/PSModule/Auto-Configure/">Auto&#8209;Configure</a></td>
        <td>
            Automates the configuration of a GitHub repo.
            <br>
            <a href="https://github.com/PSModule/Auto-Configure/issues"><img src="https://img.shields.io/github/issues-raw/psmodule/Auto-Configure?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Issues"></a>
            <a href="https://github.com/PSModule/Auto-Configure/pulls"><img src="https://img.shields.io/github/issues-pr-raw/psmodule/Auto-Configure?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Pull Requests"></a>
            <a href="https://github.com/PSModule/Auto-Configure/stargazers"><img src="https://img.shields.io/github/stars/psmodule/Auto-Configure?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Stars"></a>
            <a href="https://github.com/PSModule/Auto-Configure/watchers"><img src="https://img.shields.io/github/watchers/psmodule/Auto-Configure?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Watchers"></a>
            <a href="https://github.com/PSModule/Auto-Configure/forks"><img src="https://img.shields.io/github/forks/psmodule/Auto-Configure?style=flat-square&label=&labelColor=rgba(0%2C%200%2C%200%2C%200)&color=rgba(0%2C%200%2C%200%2C%200)&logo=data:image/svg%2bxml;base64," alt="GitHub Forks"></a>
        </td>
        <td>
            <a href="https://github.com/PSModule/Auto-Configure/releases/latest"><img src="https://img.shields.io/github/v/release/psmodule/Auto-Configure?style=flat-square&logo=github&logoColor=a0a0a0&label=&labelColor=505050&color=blue" alt="GitHub release (with filter)"></a>
        </td>
    </tr>
'@
$actionTableTemplate = @'
<!-- ACTIONS_LIST_START -->

<table>
    <tr>
        <th width="10%">Name</th>
        <th width="80%">Description</th>
        <th width="10%">Version</th>
    </tr>
    {0}
</table>

<!-- ACTION_LIST_END -->
'@
$actionTable = $actionTableRowTemplate -f 'Auto-Configure'

$Path = '.\profile\README.md'
$startSegment = '<!-- MODULE_LIST_START -->'
$endSegment = '<!-- MODULE_LIST_END -->'
$currentContent = Get-Content -Path $Path

$startIndex = $currentContent.IndexOf($startSegment)
$endIndex = $currentContent.IndexOf($endSegment)

if ($startIndex -lt 0) {
    throw 'The start comment segment was not found in the file.'
}
if ($endIndex -lt 0) {
    throw 'The end comment segment was not found in the file.'
}
if ($endIndex -lt $startIndex) {
    throw 'The end comment segment was found before the start comment segment.'
}

$updatedContent = $currentContent[0..$startIndex] + $Content + $currentContent[($endIndex)..($currentContent.Length - 1)]
Set-Content -Path $Path -Value $updatedContent
